  ...
  async fetch(req, env) {
    ...
    if (url.pathname === "/signup") {
      ...
      try {
        ...
        ).bind(b.name, b.email, b.password, "student").run();

        return Response.json({ success: true });
      } catch (e) {
        return Response.json({ success: false, error: "User exists" });
      }
    }
    ...
    if (url.pathname === "/add-result") {
      ...
      ).bind(b.student_id, b.grade, b.subject, b.result_grade, b.cgpa).run();
    ...
    if (url.pathname === "/student-data") {
      ...
      const data = await env.DB.prepare(`
        SELECT DISTINCT r.grade, p.id as payment_id, p.name, p.amount
        FROM results r
    ...
    if (url.pathname === "/memo") {
      ...
      const p = await env.DB.prepare(
        "SELECT name,amount FROM payment_items WHERE id=?"
      ).bind(pid).first();
